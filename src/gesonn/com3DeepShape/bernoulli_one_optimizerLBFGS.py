...
class Geo_Net:
    ...
    def __init__(self, **kwargs):
        ...
        self.force_switch_to_LBFGS_after_n_epochs = kwargs.get(
            "force_switch_to_LBFGS_after_n_epochs", 40
        )
    ...
    def train(self, **kwargs):
        ...
        self.scheduler = ExponentialLR(self.optimizer, 0.99)
        ...
        for epoch in range(epochs):
            ...
            if epoch == 0:
                ...
                if (
                    ...
                    and len(self.loss_history) > n1
                ):
                    condition1 = (
                            (self.loss.item() < initial_loss / self.LBFGS_switch_ratio)
                            and (
                                sum(self.loss_history[-n2:-1])
                                - sum(self.loss_history[-n1 : -n1 + n2])
                                > 0
                            )
                        )
                    condition2 = epoch > self.force_switch_to_LBFGS_after_n_epochs
                    # print(condition1, condition2)
                    # print(epoch, self.force_switch_to_LBFGS_after_n_epochs)
                    if LBFGS_activated := condition1 or condition2:
                        print(f"switching to LBFGS at epoch {epoch}, loss={self.loss:3.2e}")
                        self.create_LBFGS_optimizer()
            ...
            if self.loss.item() < best_loss_value:
                string = f"epoch {epoch: 5d}:    best loss = {self.loss.item():5.2e}, current lr = {self.scheduler.get_lr()[0]:5.2e}"
                if LBFGS_activated:
                ...
                if LBFGS_activated:
                    best_LBFGS_optimizer = copy.deepcopy(
                        self.LBFGS_optimizer.state_dict()
                    )
                else:
                    best_LBFGS_optimizer = None
        tps2 = time.time()

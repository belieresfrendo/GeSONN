...
class Geo_Net:
    ...
    def make_collocation(self, n_collocation):
        ...
        self.x_collocation = self.rho_collocation * torch.cos(theta_collocation)
        self.y_collocation = self.rho_collocation * torch.sin(theta_collocation)
    ...
    def train(self, **kwargs):
        ...
        p = torch.tensor([40.0], device=device, requires_grad=True)
        ...
        for epoch in range(epochs):
            ...
            if n_collocation > 0 :
                ...
                dirichlet_loss = (0.5 * grad_u_2).sum() / n_collocation * self.Vol

                # Contrainte
                F = (self.get_pen(self.x_collocation_min, self.y_collocation_min)).sum()/n_collocation

                dual_loss = p.item()*F

                self.loss = dirichlet_loss + dual_loss
            ...
            # Problème dual
            p = (p + 0.1 * F)
            print(f"Contrainte : {F: 5.2e}, multiplicateur de lagrange : {p.item():5.2e}")
            print(f"Energie de Dirichlet : {dirichlet_loss: 5.2e}")
            print(f"Contribution duale : {dual_loss: 5.2e}")

...
class PDE_Forward(nn.DataParallel):
    ...
    def __init__(self, layer_sizes):
        ...
        for l1, l2 in zip(layer_sizes[:-1], layer_sizes[1:]):
            layers += [nn.Linear(l1, l2).double(), STanh(l2).double()]
    ...
    def train(self, **kwargs):
        ...
        for epoch in range(epochs):
            ...
            if n_collocation > 0:
                ...
                pen = 1e2
                res = self.get_res(self.x_collocation, self.y_collocation)

                dirichlet_loss = 0.5 * grad_u_2 - fu
                self.loss = (dirichlet_loss + pen * res ** 2).sum() / n_collocation * self.Vol
